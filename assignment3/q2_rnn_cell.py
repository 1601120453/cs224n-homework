...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable('W_x', [self.input_size, self.state_size], initializer=weight_init)
            W_h = tf.get_variable('W_h', [self.state_size, self.state_size], initializer=weight_init)
            b   = tf.get_variable('b',   [self.state_size, ], initializer=bias_init)
            new_state = tf.sigmoid(tf.matmul(inputs, W_x) + tf.matmul(state, W_h) + tf.expand_dims(b, axis=0))
            ### END YOUR CODE ###
