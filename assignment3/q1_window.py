...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in xrange(len(labels)):
            features = []
            [features.extend(sentence[i+j]) for j in xrange(2*window_size+1)]
            sentence_windows = sentence_windows + [(features, labels[i])]
        windowed_data = windowed_data + sentence_windows
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        params = tf.Variable(self.pretrained_embeddings, name='embedding_matrix')
        features = tf.nn.embedding_lookup(params, self.input_placeholder)
        embeddings = tf.reshape(\
            features, [-1, self.config.n_window_features * self.config.embed_size], name='embeddings')
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W  = tf.get_variable('W', [n_features * embed_size, hidden_size], initializer=weight_init)
        U  = tf.get_variable('U', [hidden_size, n_classes], initializer=weight_init)
        b1 = tf.get_variable('b1', [1, hidden_size], initializer=bias_init)
        b2 = tf.get_variable('b2', [1, n_classes], initializer=bias_init)
