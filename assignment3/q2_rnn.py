...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        pad_sentence = sentence + max_length * [zero_vector]
        pad_labels = labels + max_length * [zero_label]
        mask = [True] * len(labels) + [False] * max_length
        ret = ret + [(pad_sentence[:max_length], pad_labels[:max_length], mask[:max_length])]
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(\
            features, [-1, self.max_length, self.config.n_features * self.config.embed_size], name='embeddings')
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable('b2', [n_classes], initializer=bias_init)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                pred = tf.matmul(h_drop, U) + tf.expand_dims(b2, axis=0)
                preds = preds + [pred]
                ### END YOUR CODE
