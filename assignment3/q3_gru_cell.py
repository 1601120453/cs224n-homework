...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_z = tf.get_variable('U_z', [self.input_size, self.state_size], initializer=weight_init)
            W_z = tf.get_variable('W_z', [self.state_size, self.state_size], initializer=weight_init)
            U_r = tf.get_variable('U_r', [self.input_size, self.state_size], initializer=weight_init)
            W_r = tf.get_variable('W_r', [self.state_size, self.state_size], initializer=weight_init)
            U_o = tf.get_variable('U_o', [self.input_size, self.state_size], initializer=weight_init)
            W_o = tf.get_variable('W_o', [self.state_size, self.state_size], initializer=weight_init)
            b_z = tf.get_variable('b_z', [self.state_size, ], initializer=bias_init)
            b_r = tf.get_variable('b_r', [self.state_size, ], initializer=bias_init)
            b_o = tf.get_variable('b_o', [self.state_size, ], initializer=bias_init)
            z = tf.sigmoid(tf.matmul(inputs, U_z) + tf.matmul(state, W_z) + tf.expand_dims(b_z, 0))
            r = tf.sigmoid(tf.matmul(inputs, U_r) + tf.matmul(state, W_r) + tf.expand_dims(b_r, 0))
            o = tf.tanh(tf.matmul(inputs, U_o) + r * tf.matmul(state, W_o) + tf.expand_dims(b_o, 0))
